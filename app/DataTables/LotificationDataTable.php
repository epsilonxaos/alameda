...
class LotificationDataTable extends DataTable
{
	/**
	...
	public function dataTable(QueryBuilder $query): EloquentDataTable
	{
		return (new EloquentDataTable($query))
			->setRowId('id')
			->addColumn('lote', function (Lotification $item) {
				return "Lote #" . $item->lote;
			})
			->addColumn('acciones', function (Lotification $item) {
			...
			->addColumn('status', function (Lotification $item) {
				...
				$formTemplate = '
					<form action="' . $ruta . '" method="post">
						...
						<button class="w-[22px] h-[22px] rounded bg-%s flex items-center justify-center mr-1" onclick="cambiar_status_lotificacion(%d)">
							%s
						</button>
					</form>';
				$status .= sprintf($formTemplate, 1, 'green-600', 1, ($item->status == 1 ? $encendido : $apagado));
				$status .= sprintf($formTemplate, 2, 'yellow-600', 2, ($item->status == 2 ? $encendido : $apagado));
				$status .= sprintf($formTemplate, 3, 'red-700', 3, ($item->status == 3 ? $encendido : $apagado));
	 ...
	 */
	public function query(Lotification $model): QueryBuilder
	{
	...
	{
		return 'Lotification_' . date('YmdHis');
	}
}
